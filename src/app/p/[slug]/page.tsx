...
export default async function PublishedPage({ params }: Props) {
  ...
  // Extract WhatsApp from footer or cta-banner
  const footerBlock = blocks.find((b) => b.type === "footer")
  const ctaBlock = blocks.find((b) => b.type === "cta-banner")
  const whatsappNumber = (footerBlock?.data?.socialLinks as Record<string, string> | undefined)?.whatsapp
  ...
  return (
    <main>
      ...
      {/* Botão flutuante WhatsApp — visível em mobile */}
      {hasWhatsApp && (
        <a
          ...
          className="fixed bottom-5 right-5 z-50 flex items-center justify-center w-14 h-14 rounded-full shadow-lg transition-transform hover:scale-105 active:scale-95"
